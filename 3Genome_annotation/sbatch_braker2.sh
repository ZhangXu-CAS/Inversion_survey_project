...
# sample id
id=$(sed -n "$((SLURM_ARRAY_TASK_ID + 1))p" braker2_ids)
echo "Start sample: $id  JOB=$SLURM_JOB_ID TASK=$SLURM_ARRAY_TASK_ID HOST=$(hostname) TIME=$(date)"

if [[ -f "${id}/${id}_hap2_renchr.fa.masked" ]]; then
    GENOME_MASK="${id}/${id}_hap2_renchr.fa.masked"
else
    GENOME_MASK="../4Annotation_done/${id}/${id}_hap2_renchr.fa"
fi
OUT_SUBDIR="${id}/braker_out2"
mkdir -p "$OUT_SUBDIR"
