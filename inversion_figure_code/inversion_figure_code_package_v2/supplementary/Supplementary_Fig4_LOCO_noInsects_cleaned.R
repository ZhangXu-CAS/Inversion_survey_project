...
ds <- build_quality_dataset(per_genome_raw, per_inv_raw, mode = 'LOCO_noInsects')
readr::write_csv(ds$per_genome, file.path(OUT_DIR, 'SuppFig4_LOCO_noInsects_per_genome.csv'))
readr::write_csv(ds$per_inv, file.path(OUT_DIR, 'SuppFig4_LOCO_noInsects_per_inversion.csv'))
build_quality_figure(ds, 'SuppFig4_LOCO_noInsects')
writeLines(capture.output(sessionInfo()), file.path(OUT_DIR, 'SuppFig4_LOCO_noInsects_sessionInfo.txt'))
