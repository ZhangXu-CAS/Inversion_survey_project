...
ds <- build_quality_dataset(per_genome_raw, per_inv_raw, mode = 'BUSCO80_DUP50')
readr::write_csv(ds$per_genome, file.path(OUT_DIR, 'SuppFig3_BUSCO80_dup50_per_genome.csv'))
readr::write_csv(ds$per_inv, file.path(OUT_DIR, 'SuppFig3_BUSCO80_dup50_per_inversion.csv'))
build_quality_figure(ds, 'SuppFig3_BUSCO80_dup50')
writeLines(capture.output(sessionInfo()), file.path(OUT_DIR, 'SuppFig3_BUSCO80_dup50_sessionInfo.txt'))
